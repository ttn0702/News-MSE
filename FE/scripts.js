...
async function fetchArticles(url) {
  try {
    const response = await fetch(
      `${domainBE}/api/get_article?url=${encodeURIComponent(url)}`
    );
...
async function randomArticle() {
  try {
    ...
    let body = { links: links };
    const currentUrl = document
      .getElementById("read-more")
      .getAttribute("data-url");
    if (currentUrl) {
      body["current_link"] = currentUrl;
    }
    const response = await fetch(`${domainBE}/api/get_recommend`, {
      method: "POST",
      headers: {
...
function readMore() {
  ...
  // save url to local storage for later use, save max 5 urls, distinct urls
  links = JSON.parse(localStorage.getItem("links")) || [];
  if (links.length >= 5) {
    links.shift();
  }
